...
case $CORE in
    ...
    v2node)
        ok "v2node config auto-generated by original installer"
        info "All config (Reality/cert/port/route) managed via panel"
        if [[ -f /etc/v2node/config.json ]]; then
            ok "Config: /etc/v2node/config.json"
        else
            err "$(t install_failed) - /etc/v2node/config.json not found"
            exit 1
        fi
        ;;
